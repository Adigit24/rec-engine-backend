...
def fetch_tmdb_movie(tmdb_id):
    url = f"https://api.themoviedb.org/3/movie/{tmdb_id}?api_key={TMDB_KEY}&append_to_response=credits,keywords"
...
def recommendations():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT tmdb_id, title, genres, overview, cast, directors, keywords, popularity, poster FROM movies")
    rows = c.fetchall()
    conn.close()
    ...
    recs = {
        ...
        "Dark / Moody / Psychological": random.sample(rows, min(10, len(rows))),
